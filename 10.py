# bot.py - HAGU BOT - Complete Version 1.5 (Gemini Integration)
import logging
import random
import time
import re
...
CHANNEL_USERNAME = "QOUTEX_FACK" # IMPORTANT: Add your channel's public username here (without @)

##### GEMINI INTEGRATION START #####
GEMINI_API_KEY = "AIzaSyCOyPQ1a6eO0N_Fboz_msazG9qC9phufo0"  # <--- এখানে আপনার জেমিনি এপিআই কী বসান
##### GEMINI INTEGRATION END #####
...
async def analyze_screenshot_with_ocr(file_path: str, p1_ign: str, p2_ign: str) -> dict:
    ...
    try:
        ...
        score_pattern = re.compile(r'(\d+)\s*[-–—]\s*(\d+)|(\d+)\s{2,}(\d+)') # Pattern for "3 - 0" or "3   0"
...
# ----------------- Universal Pre-Handler (Force Subscribe) -----------------
async def universal_pre_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, func, requires_registration=True):
    # Skip membership check for the start command itself
    if func.__name__ == 'start' or func.__name__ == 'help_command' or func.__name__ == 'rules_command': 
...
async def ask_gemini_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # A simple registration check is needed to prevent abuse
    is_registered_rec = execute_query("SELECT is_registered FROM users WHERE user_id = %s", (user.id,), 'one')
    if not is_registered_rec or not is_registered_rec[0]:
        await safe_send_message(context, user.id, "You need to register first. Please use /start.")
        return

    # Check membership for non-essential commands like /ask (optional, but good for force-subscribe bots)
    is_member = await check_channel_membership(update, context)
    if not is_member:
        keyboard = [[InlineKeyboardButton("Join Channel", url=f"https://t.me/{CHANNEL_USERNAME}")]]
        await safe_send_message(context, user.id, "Please join our channel to use the AI assistant:", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    try:
        # Initialize client with the API Key from config
        ...
        # Format the response
        response_text = f"🤖 *AI Assistant's Answer:*\n\n{response.text}"
        await status_msg.edit_text(response_text, parse_mode='Markdown')

    except APIError as e:
        logger.error(f"Gemini API Error for user {user.id}: {e}")
        await status_msg.edit_text("❌ AI service temporarily unavailable. Please ensure the API key is correct.")
    except Exception as e:
        logger.error(f"Error in ask_gemini_command: {e}")
        await status_msg.edit_text("❌ An unexpected error occurred.")
##### GEMINI INTEGRATION END #####


# ----------------- Conversation Handlers (Start, Withdraw) -----------------
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user=update.effective_user; rec=execute_query("SELECT is_registered FROM users WHERE user_id=%s",(user.id,),'one')
    if not rec: execute_query("INSERT INTO users(user_id,username,is_registered)VALUES(%s,%s,FALSE)ON CONFLICT(user_id)DO NOTHING",(user.id,user.username or user.first_name)); is_registered=False
    ...
    if not await check_channel_membership(update,context):
        await update.message.reply_text("Welcome! To start, please join our channel.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Channel", url=f"https://t.me/{CHANNEL_USERNAME}")]]))
        return ConversationHandler.END
    if is_registered: await show_main_menu(update,context,"Welcome back!"); return ConversationHandler.END
    else: await update.message.reply_text("Welcome! Please send your In-Game Name (IGN):"); return GET_IGN

async def get_ign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    execute_query("UPDATE users SET ingame_name=%s WHERE user_id=%s",(update.message.text.strip(),update.effective_user.id))
...
def main():
    ...
    reg_conv=ConversationHandler(entry_points=[CommandHandler("start",start)],states={GET_IGN:[MessageHandler(filters.TEXT & ~filters.COMMAND,get_ign)],GET_PHONE:[MessageHandler(filters.TEXT & ~filters.COMMAND,get_phone)]},fallbacks=[CommandHandler("cancel",cancel_conversation)])
    wd_conv=ConversationHandler(entry_points=[CallbackQueryHandler(lambda u,c: universal_pre_handler(u,c,withdraw_start,True),pattern='^withdraw_start$')],states={ASK_WITHDRAW_AMOUNT:[MessageHandler(filters.TEXT & ~filters.COMMAND,ask_withdraw_amount)],ASK_WITHDRAW_DETAILS:[MessageHandler(filters.TEXT & ~filters.COMMAND,ask_withdraw_details)]},fallbacks=[CommandHandler("cancel",cancel_conversation)])
    
    app.add_handler(reg_conv)
    app.add_handler(wd_conv)
    app.add_handler(CommandHandler("profile", lambda u,c: universal_pre_handler(u,c,profile_command,True)))
    app.add_handler(CommandHandler("help", help_command, block=False))
    app.add_handler(CommandHandler("rules", rules_command, block=False))
    app.add_handler(CommandHandler("support", lambda u,c: universal_pre_handler(u,c,support_command,True)))
    
    ##### GEMINI INTEGRATION START #####
    app.add_handler(CommandHandler("ask", ask_gemini_command)) # New Handler for /ask
    ##### GEMINI INTEGRATION END #####
    
    app.add_handler(CallbackQueryHandler(lambda u,c: universal_pre_handler(u,c,main_callback_router,True)))
    app.add_handler(MessageHandler(filters.TEXT | filters.PHOTO & ~filters.COMMAND, lambda u,c: universal_pre_handler(u,c,handle_text_or_photo_messages,True)))

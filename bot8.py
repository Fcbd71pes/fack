# bot.py - HAGU BOT - Complete Version 1.4.1 (SyntaxError Fixed)
import logging
import random
import time
import re
...
async def analyze_screenshot_with_ocr(file_path: str, p1_ign: str, p2_ign: str) -> dict:
    ...
    try:
        ...
        if match:
            s1 = int(match.group(1) or match.group(3))
            s2 = int(match.group(2) or match.group(4))
            
            # Simple logic: assume names are before and after the score
...
async def withdraw_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if balance<MIN_WITHDRAW:
        await update.callback_query.answer(f"❌ Min {MIN_WITHDRAW:.2f} TK to withdraw.",show_alert=True)
        return ConversationHandler.END
...
async def main_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Simple router to delegate to the right function

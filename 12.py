# bot.py - HAGU BOT - Complete Version 1.5.2 (Corrected & Secure)
import logging
import random
import time
import re
...
GEMINI_API_KEY = "AIzaSyCI3lApPqgkjpYiivBgNR3VAoPppz90Xcc" 
##### GEMINI INTEGRATION END #####
...
async def ask_gemini_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Membership check is now inside the command to avoid blocking
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Membership check is done here, inside the command
    if not await check_channel_membership(update,context):
...
def main():
    ...
    wd_conv=ConversationHandler(
        entry_points=[CallbackQueryHandler(lambda u,c: universal_pre_handler(u,c, withdraw_start, True), pattern='^withdraw_start$')],
        states={
    ...
    # Commands that do not require pre-handling (no registration or channel membership check)
    ...
    # Gemini command - it has its own internal checks
    app.add_handler(CommandHandler("ask", ask_gemini_command))

    # Commands that require pre-handling (membership and registration)
    ...
    # Callback and Message handlers that require pre-handling
    app.add_handler(CallbackQueryHandler(lambda u,c: universal_pre_handler(u,c, main_callback_router, True)))
    app.add_handler(MessageHandler(
        (filters.TEXT | filters.PHOTO) & ~filters.COMMAND,
        lambda u,c: universal_pre_handler(u,c, handle_text_or_photo_messages, True)
    ))
